...
在本书开始的一些章节中，我们有时候会以限制性的简单形式来介绍一些概念。
Haskell是一本深奥的语言，因此，一次性介绍某个主题的所有特性可能会令人难以接受。
当基础巩固之后，我们将会以这些初步介绍为基础做进一步的展开说明。

Haskell编程环境
-------------------

在Haskell语言的众多实现中，Hugs和GHC（Glasgow Haskell Compiler）被广泛应用，其中Hugs是一个解释器，主要用于教学，而GHC(Glasgow
Haskell
Compiler)在实际应用中则更加流行。与Hugs相比，GHC更加适合“实际工作”：它编译成本地代码，支持并行执行，并提供有用的性能分析工具和调试工具。正因如此，在本书中我们将采用GHC作为Haskell实现方式。

GHC主要有三个部分组成：

-  **ghc**\ 是生成本地原生代码的优化编译器。
-  **ghci**\ 是一个交互解释器和调试器。
-  **runghc**\ 是一个以脚本形式运行Haskell代码的程序，其无需对Haskell代码进行事先编译。

.. note::

    我们如何称呼GHC的各个组件

    当我们讨论整个GHC系统时，我们将称之为GHC。如果要引用到某个特定的命令，我们会直接用其名字标识，比如\ **ghc**\ ，\ **ghci**\ ，\ **runghc**\ 。

在本书中，我们假定你至少在使用6.8.2版本的GHC，这个版本是2007年发布的。大多数例子不要额外的修改也能在老版本上运行。然而，我们\ *建议*\ 使用最新版本。如果你是Windows或者Mac
OS
